...
sentinel-eval has no framework app to instrument, so the specific
...
effect, so anything that imports `sentinel_eval.observability` (directly or
...
tracer = trace.get_tracer("sentinel_eval")
